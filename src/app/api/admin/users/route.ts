...
async function checkUserListPermission(): Promise<{ isAuthorized: boolean; error?: NextResponse }> {
  ...
  try {
    ...
    const hasPermission = currentUserRoles.some(role => 
      [UserRole.ADMIN, UserRole.COORDINATOR, UserRole.MEMBER].includes(role)
    );

    if (!hasPermission) {
      return { isAuthorized: false, error: NextResponse.json({ error: "Insufficient permissions - Member, Coordinator or Admin required" }, { status: 403 }) };
    }
