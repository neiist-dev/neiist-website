  ...
  try {
    ...
    if (!result.success) {
      if (result.error === "member_of_department") {
        return NextResponse.json({ error: "member_of_department" }, { status: 403 });
      }
      return NextResponse.json({ error: "Erro interno do servidor" }, { status: 500 });
    }

    try {
      const photoPath = path.join(process.cwd(), "data", "fenix_cache", `${jwtUser.istid}.png`);
      await fs.unlink(photoPath);
    } catch {
      // Photo may not exist; non-fatal
    }
