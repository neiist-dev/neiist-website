...
type Props = {
  ...
  onFormChange?: (_field: string, _value: string) => void;
};

export default function PersonalInfoCard({ user, editing, formData, onFormChange }: Props) {
  useEffect(() => {
          ...
          {editing ? (
            ...
            <div className={styles.value}>
              {user.preferredContactMethod === "email"
                ? "Email Principal"
                : user.preferredContactMethod === "alternativeEmail"
                  ? "Email Alternativo"
