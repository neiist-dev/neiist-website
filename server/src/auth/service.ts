...
import { FenixPerson, FenixRole, UserData } from "./dto";
...
const getPersonInformation = async (accessToken: string): Promise<FenixPerson | null> => {
  try {
...
const isCoordenator = async (username: string) => {
  const isCoordenator = await collaboratorsRepository.checkCoordenator(username);
...
const getAcronyms = (personInformation: FenixPerson) => {
  ...
  personInformation.roles.forEach((role) => {
