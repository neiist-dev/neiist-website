...
storeRouter.get("/products/:id/check-availability", (req, res) => {
  try {
    ...
    const isAvailable = storeService.checkVariantAvailability(id, size as string);
 ...
 */
storeRouter.get("/orders/:id/details", collaboratorMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
...
storeRouter.post("/orders", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: (error as any).message || "Failed to create order" });
  }
});
 ...
 */
storeRouter.post("/orders/:id/pay", collaboratorMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res
      .status(500)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .json({ error: (error as any).message || "Failed to mark order as paid" });
  }
});
 ...
 */
storeRouter.post("/orders/:id/deliver", collaboratorMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res
      .status(500)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .json({ error: (error as any).message || "Failed to mark order as delivered" });
  }
});
 ...
 */
storeRouter.patch("/orders/:id/status", collaboratorMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res
      .status(500)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .json({ error: (error as any).message || "Failed to update order status" });
  }
});
...
storeRouter.delete("/orders/:id", collaboratorMiddleware, async (req, res) => {
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    res.status(500).json({ error: (error as any).message || "Failed to delete order" });
  }
});
...
storeRouter.post("/orders/export", collaboratorMiddleware, async (req, res) => {
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    res.status(500).json({ error: (error as any).message || "Failed to export orders" });
  }
});
