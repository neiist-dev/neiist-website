...
import { Order, OrderItem, OrderWithItems } from "./dto";
...
const deleteOrder = async (orderId: string) => { // TODO: check this
  const client = await getClient();
  try {
    await client.query("BEGIN");
    await client.query(
    ...
    await client.query("ROLLBACK");
    if (isDatabaseException(err) && err.code === PostgresError.FOREIGN_KEY_VIOLATION) {
      console.error("Erro ao deletar pedido:", err);
      throw new Error("Não é possível deletar o pedido, pois ele está vinculado a outros registros.");
    } else {
      console.error("Erro ao deletar pedido:", err);
      throw err;
    }
...
const markOrderAsDelivered = async (orderId: string, deliveryResponsible: string) => {
  try {
    await query(
      `UPDATE orders 
         SET delivered = true, 
