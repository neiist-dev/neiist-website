...
fs.readFile(path.resolve(CREDENTIALS_PATH), (err: NodeJS.ErrnoException | null, content: Buffer) => {
...
function authorize(credentials: Credentials, callback: (auth: Auth.OAuth2Client) => void): void {
...
function getAccessToken(oAuth2Client: Auth.OAuth2Client, callback: (auth: Auth.OAuth2Client) => void): void {
  ...
  rl.question('Enter the code from that page here: ', (code: string) => {
    oAuth2Client.getToken(code, (err: Error | null, token: Auth.Credentials | null | undefined) => {
      ...
      fs.writeFile(TOKEN_PATH as string, JSON.stringify(token), { mode: 0o600 }, (err: NodeJS.ErrnoException | null) => {
