...
import { CreateElectionDto, Election, ElectionWithOptions, Option, OptionWithVotes, Vote } from './dto';
...
const addOption = async (electionId: number, optionName: string) => {
  try {
    return await query<Option[]>('INSERT INTO options(name, "electionId") VALUES($1, $2)', optionName, electionId);
  } catch (err) {
    console.error(err);
  }
};
...
const createElection = async (election: CreateElectionDto) => {
  try {
    const createdElectionResult = await query('INSERT INTO elections(name, "startDate", "endDate") VALUES($1, $2, $3) RETURNING *', election.name, election.startDate, election.endDate);
...
const getOptions = async (electionId: number) => {
  try {
    const optionsResult = await query<Option>('SELECT * FROM options WHERE "electionId"=$1', electionId);
...
const getActiveElections = async (currDate: Date) => {
  try {
    const electionsResult = await query<Election>('SELECT * FROM elections WHERE $1 BETWEEN "startDate" AND "endDate" ', currDate);
    const elections = electionsResult.rows;
    return await Promise.all(elections.map(async (election) => ({
      ...election,
...
const createVote = async (electionId: number, vote: Vote) => {
  try {
    await query('INSERT INTO votes values($1, $2, $3)', vote.username, electionId, vote.optionId);
  } catch (err) {
    console.error(err);
  }
};
