import { Member, MemberBase, MemberInfo, NameEmailCourses } from "./dto";
import { membersRepository } from "./repository";

const waitingPeriod = 4;
const validPeriod = 12;
const gracePeriod = 6;
const dateColumns = ['registerDate', 'canVoteDate', 'renewStartDate', 'renewEndDate'] as const;
...
const getMember = async (username: string) => {
  ...
  dateColumns.forEach((date) => ret[date] = formatDate(memberInfo[date]));
...
const getActiveMembers = async () => {
  ...
  const activeMembers = await membersRepository.getActiveMembers(currDate, limitDate);
  ...
  return activeMembers
    .map(member => {
      ...
      dateColumns.forEach((date) => ret[date] = formatDate(member[date]));
      return ret as MemberInfo;
    });
};
...
const renovateMember = async (username: string, nameEmailCourses: NameEmailCourses) => {
  const memberInfo = await membersRepository.getMember(username);
  if (!memberInfo) return null;
