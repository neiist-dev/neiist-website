...
const getParsedTheses = async (theses: string) => {
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	const parsedTheses: any[] = [];
	...
	try {
		const handler = new htmlparser.DomHandler(
			(error, dom) => {
				 ...
				 * */
				// biome-ignore lint/suspicious/noExplicitAny: <explanation>
				const tableBody = dom[1] as any;

				// biome-ignore lint/suspicious/noExplicitAny: <explanation>
				// biome-ignore lint/complexity/noForEach: <explanation>
				tableBody?.children?.forEach((element: any) => {
					if (element.type === "tag" && element.name === "tr") {
						...
						// biome-ignore lint/suspicious/noExplicitAny: <explanation>
						// biome-ignore lint/complexity/noForEach: <explanation>
						trChild.forEach((subelement: any) => {
							if (subelement.type === "tag" && subelement.name === "td") {
								...
								if (i === 5) {
									...
									for (let j = 1; j < elementsNumber; j += 2) {
										arraySupervisors.push(
											subelement.children[j].children[0].data,
										);
									}
